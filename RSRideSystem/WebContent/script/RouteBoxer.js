...
RouteBoxer.prototype.buildGrid_ = function (vertices, range) {
  ...
  console.log(JSON.stringify(routeBounds.getNorthEast()));
  console.log(JSON.stringify(routeBounds.getSouthWest()));
...
RouteBoxer.prototype.findIntersectingCells_ = function (vertices) {
  ...
  console.log("findIntersectingCells_ - Vertex-Cell Cordinate[0]"+JSON.stringify(vertices[0])+":"+hintXY);
  ...
  for (var i = 1; i < vertices.length; i++) {
    ...
    console.log("findIntersectingCells_ - Vertex-Cell Cordinate["+i+"]"+JSON.stringify(vertices[i])+":"+gridXY);
...
//  console.log("getCellCoords_ - Vertex-Cell Cordinate"+JSON.stringify(latlng)+":"+(x-1)+","+(y-1));
  return ([x - 1, y - 1]);
};
...
RouteBoxer.prototype.getGridCoordsFromHint_ = function (latlng, hintlatlng, hint) {
  console.log("latlng,hintlatlng,hint[x][y]:"+JSON.stringify(latlng)+":"+JSON.stringify(hintlatlng)+":"+hint[0]+","+hint[1]);
  var x, y;
  if (latlng.lng() > hintlatlng.lng()) {
	  console.log("If - latlng.lng() > hintlatlng.lng()");
    for (x = hint[0]; this.lngGrid_[x + 1] < latlng.lng(); x++) {
    	console.log("x:"+x);	
    }
  } else {
	  console.log("Else - latlng.lng() > hintlatlng.lng()");
  ...
  if (latlng.lat() > hintlatlng.lat()) {
	  console.log("latlng.lat() > hintlatlng.lat()");
    for (y = hint[1]; this.latGrid_[y + 1] < latlng.lat(); y++) {
    	console.log("y:"+y);	
    }
    
  } else {
	  console.log("Else - latlng.lat() > hintlatlng.lat()");
    for (y = hint[1]; this.latGrid_[y] > latlng.lat(); y--) {
    	console.log("y:"+y);
    }
    
  }
 // console.log("getGridCoordsFromHint_ - Vertex-Cell Cordinate"+JSON.stringify(latlng)+":"+(x)+","+(y));
  return ([x, y]);
};
...
RouteBoxer.prototype.getGridIntersects_ = function (start, end, startXY, endXY) {
  ...
  if (end.lat() > start.lat()) {
    ...
    for (i = startXY[1] + 1; i <= endXY[1]; i++) {
      ...
      console.log("edgePoint:"+JSON.stringify(edgePoint));
